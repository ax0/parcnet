...
use crate::pod::{custom_statement::{AnchoredKeyPattern, AnonCustomStatement, ConstOrVar, GeneralisedStatementRef, ProtoGeneralisedStatement}, statement::{AnchoredKey, ProtoStatement}, Origin};

use super::{
    custom_statement::{GeneralisedStatement, ProtoCustomStatement}, entry::Entry, gadget::GadgetID, statement::{Statement, StatementOrRef, StatementRef}, value::ScalarOrVec, Op, POD
};
...
impl GeneralisedOperationWithProof<GeneralisedStatementRef> {
    pub fn deref_args(&self, table: &mut <GeneralisedStatementRef as StatementOrRef>::StatementTable) -> Result<GeneralisedOperationWithProof<GeneralisedStatement>> {
        match self {
            ...
            Self(GeneralisedOperation::Primitive(_), _) => Err(anyhow!("Primitive operations require no proof.")),
            Self(GeneralisedOperation::DeduceCustom(_, _), None) => Err(anyhow!("Custom operations require proofs."))
        }
    }
    pub fn execute(
        &self,
        ...
        self.deref_args(&mut statement_table)?
            .project().eval_with_gadget_id(gadget_id, operation_table)
    }
}
...
impl<S: Into<GeneralisedStatement> + Clone + Debug> GeneralisedOperationWithProof<S> {
    fn project(&self) -> GeneralisedOperationWithProof<GeneralisedStatement> {
        let pf = &self.1;
        let projected_pf = pf.as_ref().map(
            |v| v.iter().map(
                |op_with_pf| op_with_pf.project()
                )
            .collect::<Vec<_>>());
                match self {
                    Self(GeneralisedOperation::Primitive(op), _) => GeneralisedOperationWithProof::<GeneralisedStatement>(GeneralisedOperation::Primitive(op.clone().project()), projected_pf),
                    Self(GeneralisedOperation::DeduceCustom(op_name, op_args), _) =>GeneralisedOperationWithProof::<GeneralisedStatement>(
                        GeneralisedOperation::DeduceCustom(op_name.to_string(), op_args.iter().map(|s| s.clone().into()).collect()),
                        projected_pf)
                }
    }
}
...
impl GeneralisedOperationWithProof<GeneralisedStatement> {
    ...
    pub fn eval_with_gadget_id(
        ...
        match self {
            Self(GeneralisedOperation::Primitive(op), None) => op.generalised_eval_with_gadget_id(gadget_id),
            Self(GeneralisedOperation::DeduceCustom(op_name, op_args), Some(pf)) => {
                let op_prototype = operation_table.get(op_name).ok_or(anyhow!("Prototype for custom operation {} missing from table.", op_name))?;
                let statement_args = op_args.iter().map(
                    |arg| match Into::<GeneralisedStatement>::into(arg.clone()) {
                        GeneralisedStatement::Primitive(Statement::ValueOf(anchkey, _)) => Ok(anchkey),
...
fn custom_statement_test() -> Result<()> {
    ...
    let pf_trace = vec![
            gowp(GOp::Primitive(<Op<GSR>>::NewEntry(Entry::new_from_scalar("Constant", GoldilocksField(2)))), None),
            gowp(GOp::Primitive(<Op<GSR>>::ProductOf(
                GSR::new("POD2", "Pap"),
    ...
    let op_with_proof = gowp(
        GOp::DeduceCustom("ISDOUBLE".to_string(),
                          vec![
                              GSR::new("POD2", "Pap"),
                              GSR::new("POD1", "Pop")
                          ]
    ...
    op_with_proof.execute(
        GadgetID::ORACLE, &statement_table, &operation_table
    )?;
